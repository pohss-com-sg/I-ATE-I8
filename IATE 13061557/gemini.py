...
def roles(number):
    ...
    if number == 1:
        ...
        bot_role_plan = (
            ...
            "In your answers, create a Python dictonary with the following format: 'MONDAY': recipes, 'TUESDAY': recipes, 'WEDNESDAY': recipes, 'THURSDAY': recipes, 'FRIDAY': recipes, 'SATURDAY': recipes, 'SUNDAY': recipes. In the form of 'Date': {'BREAKFAST/LUNCH/DINNER' :{ 'name'. 'ingredients','recipe'}}"
...
# def meal_plan_spilt(type, response): #DONE IN FLASK APP.PY
...
def bot(personality, prompt): #since it returns two values, i is just there to recieve and used for the next part in spilting
    #THIS IS INSPIRED FROM GEMINI WITH STREAMLIT
    msg_no = 0
    response_no = 0
    ...
    #     response = meal_plan_spilt(type, response.text)
    print(personality)
    print(response.text)
