//since it is a lot of js i put in a diff file
path = "/static/needed.json"
//jsonfile by gemini --> https://g.co/gemini/share/6e9909e9d1a2
...
function dropdown(id) {
    ...
    const dropdown_container2 = document.getElementById(`drop-${id}`);
    const arrowdown2 = document.getElementById(`arrow-down-${id}`);
    const arrowup2 = document.getElementById(`arrow-up-${id}`);
...
window.onclick = function(event) {
    ...
    if(!event.target.closest(".dbbutton") && !event.target.closest(".dropdown-container")) { //idk why target.matches doesnt work correctly btw (very buggy) so imma just use closest
        const dropdown_container = document.getElementsByClassName("dropdown-container");
        const arrowdown = document.getElementsByClassName("fa-caret-down");
        const arrowup = document.getElementsByClassName("fa-caret-up")
;
        for (let i = 0; i < dropdown_container.length; i++) {
            if (dropdown_container[i].style.display != "none") {
                dropdown_container[i].style.display = "none" ;
                arrowdown[i].style.display = "block";
                arrowup[i].style.display = "none"; //change arrowdown to up to show the dropdown --> see if we can put a slow transition here maybe we can make it look betteter
                console.log("closed" + `${i}`) //check whether all dropdown is closed
